...
def main():
    ...
    q_pts_array= np.ndarray((num_pts,1,2), dtype=np.float32) # e a np array of the query poins to feed find
    t_pts_array = np.ndarray((num_pts,1,2), dtype=np.float32) # e a np array of the target poins to feed find
    ...
    for idx_match, match in enumerate(matches):
        ...
        t_x = t_key_points[q_idx].pt[0]
        t_y = t_key_points[q_idx].pt[1]
        t_pts_array[idx_match,0,0] = q_x
        t_pts_array[idx_match,0,1] = q_y
    ...
    #                flags = 2)
    # img3 = cv2.drawMatches(q_image,q_key_points,t_image,t_key_points,matches,None,**draw_params)
    ...
    #stitch image
    #Alt 1:
    #stitched_image = ((t_image.astype(float)+q_image.astype(float))/2).astype(np.uint8)

    #Alt 2:
    # overlap_mask = q_image_warped > 0
    # stitched_image = copy.deepcopy(t_image)
    # stitched_image[overlap_mask] = q_image_warped[overlap_mask]
    
    #Alt 3:
    overlap_mask = q_image_warped > 0
    stitched_image = copy.deepcopy(t_image)
    stitched_image[overlap_mask] = ((q_image_warped[overlap_mask].astype(float) + stitched_image[overlap_mask].astype(float))/2).astype(np.unit8)
    ...
    cv2.imshow('test',q_image_warped)

    cv2.namedWindow('test2',cv2.WINDOW_NORMAL)
    cv2.imshow('test2',t_image)

    cv2.namedWindow('test3',cv2.WINDOW_NORMAL)
    cv2.imshow('test3',stitched_image)
    # ------------------------------------------
    # Termination
    # ------------------------------------------
