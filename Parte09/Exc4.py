...
def main():
    ...
    possible_values = set(cluster_idxs)   # lista sem repetições
    possible_values.remove(-1)
    print(possible_values)

    largest_clouster_num_points = 0
    largest_cluster_idx = None  
    for value in possible_values:
        num_points = cluster_idxs.count(value)
        if num_points > largest_clouster_num_points:
            largest_cluster_idx = value
            largest_clouster_num_points = num_points
    
 
    largest_idxs = list(locate(cluster_idxs, lambda x: x == largest_cluster_idx))    #max_label = labels.max()
