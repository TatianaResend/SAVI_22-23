...
def main():
    ...
    plane_model, inliers_idxs = point_cloud.segment_plane(distance_threshold=0.3,
                                         ransac_n=3,
                                         num_iterations=100)
    [a, b, c, d] = plane_model
    print('Plane equation: ' + str(a) +  ' x + ' + str(b) + ' y + ' + str(c) + ' z + ' + str(d) + ' = 0' )
    
    inlier_cloud = point_cloud.select_by_index(inliers_idxs)
    inlier_cloud.paint_uniform_color([1.0, 0, 0]) #paintd the plane in red
    outlier_cloud = point_cloud.select_by_index(inliers_idxs, invert=True)
